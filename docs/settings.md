...
- `automation_provider_write_enabled`: second switch for future provider-side automation. Provider writes require this to be `true` and `automation_level` to be `provider_write`. Manifest purge tools remain separate.
