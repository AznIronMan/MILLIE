                ...
                self.write_json(
                    {
                        ...
                        "data_dir": str(self.app.config.data_dir),
                    }
                )
...
class MillieHTTPServer(ThreadingHTTPServer):
    def __init__(self, server_address: tuple[str, int], config: AppConfig, db: MillieDatabase):
...
def run_server(config: AppConfig) -> None:
    ...
    db = MillieDatabase(resolved.db_path, resolved.data_dir)
    db.init()
    server = MillieHTTPServer((resolved.host, resolved.port), resolved, db)
    print(f"MILLIE listening on http://{resolved.host}:{resolved.port}")
    print(f"Database: {resolved.db_path}")
    print(f"Data dir: {resolved.data_dir}")
